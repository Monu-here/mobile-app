...
export default function ReligionScreen({ onBack }) {
  ...
  const fetchReligions = async () => {
    ...
    setError(null);
    try {
      const result = await apiService.getReligions();
  ...
  const handleAdd = async () => {
    if (!name.trim()) {
      showToast('Please enter religion name', 'error');
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await apiService.updateReligion(editingId, payload);
        showToast('Religion updated successfully', 'success');
      } else {
        await apiService.addReligion(payload);
      ...
      showToast(err?.message || 'Failed to save religion', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
          ...
          >
            {submitting ? (
              ...
              <Text style={styles.submitText}>
                {editingId ? 'Update Religion' : 'Add Religion'}
              </Text>
            )}
          </TouchableOpacity>
          {editingId && (
      ...
      <View style={styles.listContainer}>
        {loading ? (
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ color: '#999', fontSize: 16 }}>No religions found</Text>
          </View>
        ) : (
          <FlatList
            data={religions}
