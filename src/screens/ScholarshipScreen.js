...
export default function ScholarshipScreen({ onBack }) {
  ...
  const fetchScholarships = async () => {
    ...
    setError(null);
    try {
      const result = await apiService.getScholarships();
  ...
  const handleAdd = async () => {
    if (!name.trim()) {
      showToast('Please enter scholarship name', 'error');
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        await apiService.addScholarship(payload);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.listContainer}>
        {loading ? (
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ color: '#999', fontSize: 16 }}>No scholarships found</Text>
          </View>
        ) : (
          <FlatList
            data={scholarships}
